...
type FuryintConfig struct {
	TrustingPeriod time.Duration
	MaxClockDrift  time.Duration
}

func (tmcfg *FuryintConfig) GetClientType() string {
	return exported.Furyint
}

var _ ClientConfig = &FuryintConfig{}

// TestChainFuryint is a testing struct that 'wraps' a TestChain with the last DM Header,
// the current ABCI header.
type TestChainFuryint struct {
	TC *TestChain
...
var _ TestChainClientI = &TestChainFuryint{}

// NewChainFuryintClient initializes the consunsus spesisifc pare of the TestChain
func NewChainFuryintClient(tc *TestChain) *TestChainFuryint {
...
func (chain *TestChainFuryint) GetSelfClientType() string {
	return exported.Furyint
}

func (chain *TestChainFuryint) NewConfig() ClientConfig {
	return &FuryintConfig{
...
func (chain *TestChainFuryint) GetContext() sdk.Context {
...
func (chain *TestChainFuryint) NextBlock() {
...
func (chain *TestChainFuryint) CurrentDMClientHeader() *ibcdmtypes.Header {
...
func (chain *TestChainFuryint) CreateDMClientHeader(chainID string, blockHeight int64, trustedHeight clienttypes.Height, timestamp time.Time, tmValSet, tmTrustedVals *tmtypes.ValidatorSet, signers []tmtypes.PrivValidator) *ibcdmtypes.Header {
	var (
	...
	// Furyint check the header bytes signatures
...
func (chain *TestChainFuryint) UpdateCurrentHeaderTime(t time.Time) {
	chain.CurrentHeader.Time = t
}

// BeginBlock signals the beginning of a block with chain.CurrentHeader
func (chain *TestChainFuryint) BeginBlock() {
...
func (chain *TestChainFuryint) ClientConfigToState(clientConfig ClientConfig) exported.ClientState {
	tmConfig, ok := clientConfig.(*FuryintConfig)
	require.True(chain.TC.T, ok)
...
func (chain *TestChainFuryint) GetConsensusState() exported.ConsensusState {
	return chain.LastHeader.ConsensusState()
}

func (chain *TestChainFuryint) GetLastHeader() interface{} {
