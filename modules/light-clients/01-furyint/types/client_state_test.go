...
func (suite *FuryintTestSuite) TestStatus() {
	var (
...
func (suite *FuryintTestSuite) TestValidate() {
	testCases := []struct {
		name        string
		clientState *types.ClientState
		expPass     bool
...
func (suite *FuryintTestSuite) TestInitialize() {
	var (
		furyintCounterpartyChain *ibctesting.TestChain
		endpoint                *ibctesting.Endpoint
	)
...
func (suite *FuryintTestSuite) TestVerifyClientConsensusState() {
	testCases := []struct {
		name           string
		clientState    *types.ClientState
		consensusState *types.ConsensusState
...
func (suite *FuryintTestSuite) TestVerifyConnectionState() {
	var (
		clientState                          *types.ClientState
		proof                                []byte
	...
	for _, tc := range testCases {
		...
		suite.Run(tc.name, func() {
			suite.SetupTestWithConsensusType(exported.Tendermint, exported.Furyint) // reset

			// setup testing conditions
			path := ibctesting.NewPath(suite.chainA, suite.chainB)
			suite.coordinator.Setup(path)

			if suite.chainB.TestChainClient.GetSelfClientType() == exported.Furyint {
				furyintCounterpartyChain = suite.chainA
...
// client on chainA
func (suite *FuryintTestSuite) TestVerifyChannelState() {
	var (
		clientState                          *types.ClientState
		proof                                []byte
		proofHeight                          exported.Height
...
func (suite *FuryintTestSuite) TestVerifyPacketCommitment() {
	var (
		clientState                          *types.ClientState
		proof                                []byte
...
// is simulated.
func (suite *FuryintTestSuite) TestVerifyPacketAcknowledgement() {
	var (
		clientState                          *types.ClientState
		proof                                []byte
		delayTimePeriod                      uint64
...
// no receive.
func (suite *FuryintTestSuite) TestVerifyPacketReceiptAbsence() {
	var (
		clientState                          *types.ClientState
		proof                                []byte
		delayTimePeriod                      uint64
...
// simulated.
func (suite *FuryintTestSuite) TestVerifyNextSeqRecv() {
	var (
		clientState                          *types.ClientState
		proof                                []byte
		delayTimePeriod                      uint64
