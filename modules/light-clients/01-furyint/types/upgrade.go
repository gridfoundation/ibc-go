...
// in client state that must be the same across all valid Furyint clients for the new chain.
// VerifyUpgrade will return an error if:
// - the upgradedClient is not a Furyint ClientState
...
// - any Furyint chain specified parameter in upgraded client such as ChainID,
...
func (cs ClientState) VerifyUpgradeAndUpdateState(
	...
	if !ok {
		return nil, nil, sdkerrors.Wrapf(clienttypes.ErrInvalidClientType, "upgraded client must be Furyint client. expected: %T got: %T",
			&ClientState{}, upgradedClient)
	}
	tmUpgradeConsState, ok := upgradedConsState.(*ConsensusState)
	if !ok {
		return nil, nil, sdkerrors.Wrapf(clienttypes.ErrInvalidConsensus, "upgraded consensus state must be Furyint consensus state. expected %T, got: %T",
