...
func (suite *FuryintTestSuite) TestCheckSubstituteUpdateStateBasic() {
	var (
...
func (suite *FuryintTestSuite) TestCheckSubstituteAndUpdateState() {
	testCases := []struct {
		name         string
		FreezeClient bool
		ExpireClient bool
...
func (suite *FuryintTestSuite) TestIsMatchingClientState() {
	var (
	...
	testCases := []struct {
	...
	}{
		{
			"matching clients", func() {
				...
				case exported.Furyint:
					// ChainBs' counterparty client is Furyint
				...
				case exported.Tendermint:
					// ChainAs' counterparty client is Furyint
