...
func (suite *FuryintTestSuite) TestCheckHeaderAndUpdateState() {
	var (
	...
	var chainAFuryint *ibctesting.TestChainFuryint

	testCases := []struct {
		name      string
		setup     func(*FuryintTestSuite)
		expFrozen bool
		expPass   bool
	}{
		{
			name: "successful update with next height and same validator set",
			setup: func(suite *FuryintTestSuite) {
				...
				newHeader = chainAFuryint.CreateDMClientHeader(chainID, int64(heightPlus1.RevisionHeight), height, suite.headerTime, suite.valSet, suite.valSet, signers)
		...
		},
		{
			...
			setup: func(suite *FuryintTestSuite) {
				clientState = types.NewClientState(chainID, trustingPeriod, maxClockDrift, height, commitmenttypes.GetSDKSpecs(), upgradePath)
				consensusState = types.NewConsensusState(suite.clientTime, commitmenttypes.NewMerkleRoot(suite.header.Header.GetAppHash()), suite.valsHash)
				newHeader = chainAFuryint.CreateDMClientHeader(chainID, int64(heightPlus5.RevisionHeight), height, suite.headerTime, bothValSet, suite.valSet, bothSigners)
				currentTime = suite.now
			},
			expFrozen: false,
			expPass:   true,
		},
		{
			...
			setup: func(suite *FuryintTestSuite) {
				...
				newHeader = chainAFuryint.CreateDMClientHeader(chainID, int64(heightPlus1.RevisionHeight), height, suite.headerTime, bothValSet, bothValSet, bothSigners)
				currentTime = suite.now
			},
			expFrozen: false,
			expPass:   true,
		},
		{
			...
			setup: func(suite *FuryintTestSuite) {
				...
				newHeader = chainAFuryint.CreateDMClientHeader(chainID, int64(heightMinus1.RevisionHeight), heightMinus3, suite.headerTime, bothValSet, suite.valSet, bothSigners)
				currentTime = suite.now
			},
			expFrozen: false,
			expPass:   true,
		},
		{
			...
			setup: func(suite *FuryintTestSuite) {
				...
				consensusState = types.NewConsensusState(suite.clientTime, commitmenttypes.NewMerkleRoot(suite.header.Header.GetAppHash()), suite.valsHash)
				consStateHeight = heightMinus3
				newHeader = chainAFuryint.CreateDMClientHeader(chainIDRevision0, int64(height.RevisionHeight), heightMinus3, suite.headerTime, bothValSet, suite.valSet, bothSigners)
				currentTime = suite.now
			},
			expPass: true,
		},
		...
		},
		{
			...
			setup: func(suite *FuryintTestSuite) {
				...
				consensusState = types.NewConsensusState(suite.clientTime, commitmenttypes.NewMerkleRoot(suite.header.Header.GetAppHash()), suite.valsHash)
				newHeader = chainAFuryint.CreateDMClientHeader(chainID, int64(heightPlus5.RevisionHeight), height, suite.headerTime, suite.valSet, suite.valSet, signers)
				currentTime = suite.now
		...
		},
		{
			...
			setup: func(suite *FuryintTestSuite) {
				clientState = types.NewClientState(chainID, trustingPeriod, maxClockDrift, height, commitmenttypes.GetSDKSpecs(), upgradePath)
				consensusState = types.NewConsensusState(suite.clientTime, commitmenttypes.NewMerkleRoot(suite.header.Header.GetAppHash()), suite.valsHash)
				newHeader = chainAFuryint.CreateDMClientHeader("ethermint", int64(heightPlus1.RevisionHeight), height, suite.headerTime, suite.valSet, suite.valSet, signers)
				currentTime = suite.now
			},
			expFrozen: false,
			expPass:   false,
		},
		{
			...
			setup: func(suite *FuryintTestSuite) {
				...
				consensusState = types.NewConsensusState(suite.clientTime, commitmenttypes.NewMerkleRoot(suite.header.Header.GetAppHash()), suite.valsHash)
				newHeader = chainAFuryint.CreateDMClientHeader(chainIDRevision1, 1, height, suite.headerTime, suite.valSet, suite.valSet, signers)
				currentTime = suite.now
			},
			expPass: false,
		},
		{
			...
			setup: func(suite *FuryintTestSuite) {
				...
				consensusState = types.NewConsensusState(suite.clientTime, commitmenttypes.NewMerkleRoot(suite.header.Header.GetAppHash()), suite.valsHash)
				newHeader = chainAFuryint.CreateDMClientHeader(chainIDRevision1, 3, height, suite.headerTime, suite.valSet, suite.valSet, signers)
				currentTime = suite.now
			},
			expFrozen: false,
			expPass:   false,
		...
		},
		{
			...
			setup: func(suite *FuryintTestSuite) {
				clientState = types.NewClientState(chainID, trustingPeriod, maxClockDrift, height, commitmenttypes.GetSDKSpecs(), upgradePath)
				consensusState = types.NewConsensusState(suite.clientTime, commitmenttypes.NewMerkleRoot(suite.header.Header.GetAppHash()), suite.valsHash)
				newHeader = chainAFuryint.CreateDMClientHeader(chainID, int64(heightPlus1.RevisionHeight), height, suite.now.Add(time.Minute), suite.valSet, suite.valSet, signers)
				currentTime = suite.now
			},
			expFrozen: false,
			expPass:   false,
		},
		{
			...
			setup: func(suite *FuryintTestSuite) {
				clientState = types.NewClientState(chainID, trustingPeriod, maxClockDrift, height, commitmenttypes.GetSDKSpecs(), upgradePath)
				consensusState = types.NewConsensusState(suite.clientTime, commitmenttypes.NewMerkleRoot(suite.header.Header.GetAppHash()), suite.valsHash)
				newHeader = chainAFuryint.CreateDMClientHeader(chainID, int64(heightPlus1.RevisionHeight), height, suite.clientTime, suite.valSet, suite.valSet, signers)
				currentTime = suite.now
			},
			expFrozen: false,
			expPass:   false,
		...
		},
		{
			...
			setup: func(suite *FuryintTestSuite) {
				...
				newHeader = chainAFuryint.CreateDMClientHeader(chainID, int64(heightMinus1.RevisionHeight), height, suite.headerTime, suite.valSet, suite.valSet, signers)
				currentTime = suite.now
			},
			expFrozen: false,
			expPass:   false,
	...
	for i, tc := range testCases {
		...
		suite.Run(fmt.Sprintf("Case: %s", tc.name), func() {
			suite.SetupTestWithConsensusType(exported.Furyint, exported.Tendermint) // reset
			chainAFuryint = suite.chainA.TestChainClient.(*ibctesting.TestChainFuryint)
...
func (suite *FuryintTestSuite) TestPruneConsensusState() {
